...
export function drawNoteStem(
    ...
    if (stemDirection === Direction.up) {
        ...
        return buildPath(
            ...
            [
                x + 1.115,
                y + (highest.offset / 2 - noteheadOffsetForStem) - (length - noteheadOffsetForStem)
            ]
        );
    }
    ...
    if (stemDirection === Direction.down) {
        ...
        return buildPath(
            ...
            [
                x + 0.0625,
                y + lowest.offset / 2 + noteheadOffsetForStem + (length - noteheadOffsetForStem)
            ]
        );
    }
}
